...
function startApp() {
  ...
  let diagramData = {
    classes: ["Controller", "End", "Error"],
    associations: [["Controller", "End"]],
  };

  let activeClass = "Controller";
  ...
  client.on("message", (topic, message) => {
    try {
      ...
      // On parsing error, set to Error state only if not already in Error state
      if (activeClass !== "Error") {
        activeClass = "Error";
        updateDiagram();
      }
